...
class ServerFailure extends Failure {
  ...
  factory ServerFailure.fromDioError(DioException error) {
    switch (error.type) {
      ...
      case DioExceptionType.unknown:
        return const ServerFailure(message: AppStrings.errorUnknown);
    }
  }
  factory ServerFailure.fromDioResponse(int statusCode, dynamic response) {
    if (statusCode == 404 || statusCode == 400) {
      ...
      return const ServerFailure(message: AppStrings.errorInternal);
    } else if (statusCode == 401) {
      return const ServerFailure(message: AppStrings.errorUnauthorized);
    }
    return const ServerFailure(message: AppStrings.badResponseMessage);
  }
}
